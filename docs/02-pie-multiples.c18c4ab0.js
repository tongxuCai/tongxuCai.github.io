...
function ready(datapoints) {
  ...
  container.selectAll('svg').data(nested).enter().each(function (d) {
    var datapoints = d.values;
    var svg = d3.select('#chart-2').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
    var task = ['Typing code', 'Rewriting code', 'Reading StackOverflow'];
    var pie = d3.pie().value(function (d) {
      return d.minutes;
    });
    console.log(nested); // console.log(pie(nested))

    var angleScale = d3.scaleBand().domain(task).range([0, Math.PI * 2]);
    var radius = 100;
    var arc = d3.arc().innerRadius(0).outerRadius(radius);
    var labelArc = d3.arc().innerRadius(radius).outerRadius(radius).startAngle(function (d) {
      return angleScale(d);
    }).endAngle(function (d) {
      return angleScale(d) + angleScale.bandwidth();
    });
    var colorScale = d3.scaleOrdinal().range(['pink', 'cyan', 'magenta']); // console.log(pie(datapoints))
    ...
    svg.selectAll('.outside-label').data(angleScale.domain()).enter().append('text').text(function (d) {
      return d;
    }).attr('fill', 'black').attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('transform', function (d) {
