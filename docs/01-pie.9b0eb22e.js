...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
...
var labelArc = d3.arc().innerRadius(radius).outerRadius(radius).startAngle(function (d) {
  return pie(d.minutes);
}).endAngle(function (d) {
  return pie(d.minutes) + arc.bandwidth();
});
var colorScale = d3.scaleOrdinal().range(['pink', 'cyan', 'magenta']);
