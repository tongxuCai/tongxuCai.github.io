...
var svg = d3.select('#chart-7').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
d3.csv(require('/data/time-binned.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(datapoints) {}
},{"d3":"../node_modules/d3/index.js","/data/time-binned.csv":"data/time-binned.csv"}]},{},["scripts/homework-13/07-mystery.js"], null)
//# sourceMappingURL=/07-mystery.1378a6cf.js.map