...
},{}],"scripts/homework-13/02-pieb.js":[function(require,module,exports) {
"use strict";
...
var m = 10;
var r = 100;
var z = d3.scaleOrdinal(d3.schemeCategory10); // Define a pie layout: the pie angle encodes the minutes of flights. Since our
...
var arc = d3.svg.arc().innerRadius(r / 2).outerRadius(r); // Load the flight data asynchronously.

d3.csv(require('/data/time-breakdown-all.csv'), function (error, flights) {
  if (error) throw error; // Nest the flight data by originating airport. Our data has the minutess per
  // airport and task, but we want to group minutess by aiport.

  var airports = d3.nest().key(function (d) {
    return d.project;
  }).entries(flights);
  var names = airports.map(function (d) {
    return d.name;
  });
  z.domain(names);
  console.log(flights); // Insert an svg element (with margin) for each airport in our dataset. A
  // child g element translates the origin to the pie center.

  var svg = d3.select('#chart-2').selectAll('div').data(airports).enter().append('div') // http://code.google.com/p/chromium/issues/detail?id=98951
  .style('display', 'inline-block').style('width', (r + m) * 2 + 'px').style('height', (r + m) * 2 + 'px').append('svg').attr('width', (r + m) * 2).attr('height', (r + m) * 2).append('g').attr('transform', 'translate(' + (r + m) + ',' + (r + m) + ')'); // Add a label for the airport. The `key` comes from the nest operator.

  svg.append('text').attr('dy', '.35em').attr('text-anchor', 'middle').text(function (d) {
    return d.key;
  ...
  var g = svg.selectAll('g').data(function (d) {
    return pie(d.values);
  }).enter().append('g'); // Add a colored arc path, with a mouseover title showing the minutes.
  ...
  }).append('text').attr('dy', '.35em').attr('text-anchor', 'middle').attr('transform', function (d) {
...
},{"d3":"../node_modules/d3/index.js","/data/time-breakdown-all.csv":"data/time-breakdown-all.csv"}]},{},["scripts/homework-13/02-pieb.js"], null)
//# sourceMappingURL=/02-pieb.e396271f.js.map